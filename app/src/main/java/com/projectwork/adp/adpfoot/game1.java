...
public class game1 extends android.support.v4.app.Fragment {
Button ans;
RadioButton a;
RadioGroup radioGroup;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        final View view = inflater.inflate(R.layout.game_test_fragment,container,false);
...
if (text==null) {
    Toast.makeText(v.getContext(), "LET'S GET INTERACTIVE!\nPlease select an option", Toast.LENGTH_SHORT).show();
    return;

}else {

                if (text.contains("roberto baggio")) {
                    if (mp != null ) {
                        if (!mp.isPlaying()) {
                            mp.start();

                            game2 newGamefragment = new game2();
